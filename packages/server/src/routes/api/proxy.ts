...
app.all('/:encryptedAuthAndData/:filename?', async (c) => {
  ...
  try {
    ...
    const { encryptedAuthAndData, filename } = c.req.param();
    ...
    clientIp = c.get('requestIp') || 'unknown';
    ...
    const responseHeaders = sanitiseHeaders(upstreamResponse.headers);
    ...
    c.status(upstreamResponse.statusCode as StatusCode);
    ...
    if (c.req.method === 'HEAD') {
      ...
      return stream(c, async (stream) => {
        ...
        if (sizeLimiter) {
          // Manually pipe through size limiter
          upstreamResponse!.body.pipe(sizeLimiter);
          for await (const chunk of sizeLimiter) {
            await stream.write(chunk);
          }
        } else {
          // Manually iterate over upstream body
