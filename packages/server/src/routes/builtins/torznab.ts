...
import { createLogger, TorznabAddon, fromUrlSafeBase64 } from '@aiostreams/core';
...
app.get('/:encodedConfig/manifest.json', async (c) => {
  ...
  try {
    ...
    logger.error('Failed to get torznab manifest:', error);
    throw error;
  }
});
...
app.get('/:encodedConfig/stream/:type/:id', async (c) => {
  ...
  try {
    ...
    logger.error('Failed to get torznab streams:', error);
    throw error;
  }
});
